# 2.4.4
- Improves zoom behavior
- Makes content on smaller devices scrollable (Tested with Pf2e and Dnd5e)

# 2.4.3
- Adds backward compatibility for FoundryVTT 13

# 2.4.2
- Fixes: PF2E – As a non-Sheet-Only user, opening the spell preparation sheet was handled as if the user was a sheet-only user.
- Fixes: Crash for some systems (like pf2e) regarding elements not having tooltips

# 2.4.1
- Foundry V14 compatibility
- Fixes styling issues with dnd5e

# 2.3.5
- Disables tooltips for dnd5e
- Increases list item buttons slightly for better access

# 2.3.4
- Fixes the issue on iPads and iPhones where dialogs don't expend the full height of the content

# 2.3.3
- Fixes sheet header. Header buttons (except the close button) are now displayed. 

# 2.3.1
- Fixes styling issues for smartphones

# 2.2.2
- Fixes issue with journal popout, which was not rendered on sidebar if sheet-only plus wasn't installed
- Fixes styling issues with Tidy5e sheet

# 2.2.1
- Adds [Lucas's Awesome Messenger Extension ](https://foundryvtt.com/packages/lame-messenger) to chat controls
- Journal is now available in the free version
- Better position for chat and journal

# 2.0.2
- Minor style fix for dsa5e 7.1.11 
